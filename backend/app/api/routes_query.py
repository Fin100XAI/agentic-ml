...
def _question_scout(ds, shape: dict[str, Any], source: str) -> list[dict[str, Any]] | None:
    ...
    try:
        ...
        raw = provider.complete_json(
            ...
            "too: at most 3 ranked (top/bottom) picks, NEVER both top and "
            "bottom of the same metric, and always include at least one "
            "relationship pair plus one split or count, and when a date or "
            "period column exists also trend, change, and trend_by with a "
            "small group - time data deserves time views. Varied questions "
            "make a varied board. Prefer headline measures "
